...
13. '최근 실적' H2 섹션에 연도별·분기별 수치를 텍스트로 나열하지 말 것. 연간 재무 테이블·SVG 차트·분기 실적 표는 해당 H2 바로 아래에 자동 삽입되므로, 이 섹션은 FCF 추이·재무 안정성·이익 품질 등 정성적 분석만 서술한다.
...
2문장 요약 (결론 → 근거 구조)
FCF(잉여현금흐름) 추이 평가 포함 (FCF = OCF - CAPEX, 양수면 자체 투자 여력, 음수면 외부 자금 필요)
재무 안정성·현금흐름 특이사항·이익 품질 등 정성적 분석 서술
